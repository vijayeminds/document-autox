...
const mockDocuments = Array.from({ length: 100 }, (_, i) => ({
  ...
  vendor: ['Acme Corp', 'Tech Solutions', 'Global Supplies', 'Prime Vendor'][i % 4],
  status: ['Received', 'Processing', 'Linked', 'Needs Review', 'Exception'][i % 5],
...
const mockExceptions = Array.from({ length: 20 }, (_, i) => ({
  id: `exc-${i}`,
  document_id: `doc-${i}`,
  exception_type: ['Duplicate', 'Price Variance', 'Missing PO', 'Tax Mismatch'][i % 4],
  severity: ['Low', 'Medium', 'High'][i % 3],
  created_date: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
  resolved_at: i % 3 === 0 ? new Date(Date.now() - Math.random() * 10 * 24 * 60 * 60 * 1000).toISOString() : null,
}));
...
const mockApprovals = Array.from({ length: 30 }, (_, i) => ({
  ...
  created_date: new Date(Date.now() - Math.random() * 60 * 24 * 60 * 60 * 1000).toISOString(),
}));
  ...
  entities: {
    ...
    AuditLog: {
      ...
      create: async (data: any) => {
        await new Promise(resolve => setTimeout(resolve, 200));
        return { id: `audit-${Date.now()}`, ...data, created_date: new Date().toISOString() };
      }
    }
  }
};
